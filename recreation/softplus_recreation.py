...
plt.plot(np.concatenate([x_target, x_extra]), np.concatenate([y_target, y_extra]), '--', label="softplus", color = 'red')
plt.plot(curve[:, 0], curve[:, 1], label = 'softplus approximation', color = 'k', linewidth=0.7)
...
plt.tight_layout()
plt.axis('equal')
plt.savefig('./PICS/Approx_softplus.pdf')