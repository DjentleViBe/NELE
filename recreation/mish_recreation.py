...
y_target = mish(x_target)

x_extra = np.linspace(0, 4, 200)
y_extra = mish(x_extra)
...
plt.plot(np.concatenate([x_target, x_extra]), np.concatenate([y_target, y_extra]), '--', label="mish", color = 'red')
plt.plot(curve[:, 0], curve[:, 1], label = 'mish approximation', color = 'k', linewidth=0.7)
...
#plt.title("Optimized Cubic NURBS Approximation of Gmish")
plt.xlabel("x")
plt.ylabel("f(x)")
plt.tight_layout()
plt.axis('equal')
plt.savefig('./PICS/Approx_mish.pdf')