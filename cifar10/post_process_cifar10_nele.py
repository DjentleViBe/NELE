...
def plot_only(study_type):
    ...
    for i, act in enumerate(activations_file):
        epochs, losses_train, losses_val, losses_test = csv_read2('RESULTS/' \
                                        ...
                                        + activations_file[i] + '.csv', 'epoch', 'loss', '', 'test')
        ...
        if 'nele' in act or 'lelu' in act:
            ...
            ax3.plot(epochs, losses_train, colors[i], label=activations[i], linewidth = 0.7)
...
def process_cifar10_nele():
    """
    Docstring for process_cifar10_nele
    """
    plot_only('CIFAR10')
