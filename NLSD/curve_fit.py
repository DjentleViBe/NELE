    ...
    for af in AF_NLSD:
        for afunc in cfg.FUNC_NLSD:
            ...
            if afunc == 'sine':
                ...
                ################### TRIG NOISE ##########################
                x = torch.linspace(-5, 5, 200).unsqueeze(1)
                y = torch.sin(x) + 0.5 * torch.sin(3*x) + (1 / cfg.noise_level) * torch.randn(x.size())
                ...
                y = torch.tensor(x**3) / (torch.exp(x) - 1 + 1E-6) + (1 / cfg.noise_level) * torch.randn(x.size())
                loss_val, std_val = nlsd_data(x, y, af=af, study_type='exppoly')
                loss_collect.append(loss_val)
                std_deviation_collect.append(std_val)

def curve_fit_nele(device, reset, exec):
    x = torch.linspace(-5, 5, 200).unsqueeze(1)
    if reset == 1:
        reset_directory('./RESULTS/NLSD/')
        reset_directory('./PICS/NLSD/')
