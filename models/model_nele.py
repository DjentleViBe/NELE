...
class NeleUniversalRead(nn.Module):
    """
    NELE AF with hyperparam learning, masking options, buffer and xmin clamping
    """
    def __init__(self, device, proparray, num_points = 200):
        super().__init__()
        self.num_points = num_points
        self.device = device
        ...
        self.register_buffer('N3', t**3)
        if cfg.learnable:
            self.l = nn.Parameter(torch.tensor(self.proparray["cp2"][0] * 1.4142))
            self.w1 = nn.Parameter(torch.tensor(self.proparray["w1"]))
            self.w2 = nn.Parameter(torch.tensor(self.proparray["w2"]))
            self.y1 = nn.Parameter(torch.tensor(self.proparray["cp1"][1]))
            self.x1 = nn.Parameter(torch.tensor(self.proparray["cp1"][0]))
            self.y0 = nn.Parameter(torch.tensor(0.0))
            self.x0 = nn.Parameter(torch.tensor(self.proparray["cp0"][0]))
        else:
            self.cp1 = torch.tensor(self.proparray["cp1"], device=device)
            self.cp2 = torch.tensor(self.proparray["cp2"], device=device)
            self.cp3 = torch.tensor(self.proparray["cp3"], device=device)
            self.w0 = torch.tensor(self.proparray["w0"], device=device)
    ...
    def forward(self, x):
        ...
        # return
        if self.proparray["masking"] == 0:
            return y
        elif self.proparray["masking"] == 1:
            return torch.where(x > 0, x, y)
        else:
            return torch.where(x > 0, x, torch.where(x < self.proparray["cp0"][0], torch.zeros_like(x), y))
