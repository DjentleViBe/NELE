...
are given in Appendix~\ref{app:nurbsparams}.
  ...
  \texttt{NELE} implementation is summarized in Algorithm ~\ref{alg:nele_training}. For a cubic 
  spline, there are 4 control points and their associated weights. For the learnable version of the 
  activation, a piecewise type construction is considered.
  The last control point ($p_3$) is fixed at $[0, 0]$. To maintain tangency of the curve to the postive space, 
  $p_2$ has to have a slope of 1, thus reducing parameter count to just its length $l$.
  $p_1$ and $p_0$ each contains 2 parameters for coordinates, [$x_1$, $y_1$] and [$x_0$, $y_0$] respectively. 
 Other variations are listed in Table~\ref{tab:nelevars}. The usage of buffers avoids recomputing the cubic Bernstein basis functions on every forward pass.
...
\end{table}
  \begin{table}
  \caption{Several variants of the \texttt{NELE} implementation that are provided. 
  The spline is parameterized by control points $p_n = [x_n, y_n]$ and corresponding weights $w_n$. 
  In the case of a cubic spline, four control points ($n = 4$) are employed.}
  \label{tab:nelevars}
  \begin{tabular}{@{}lllll@{}}
    \toprule
    VARIATION & Learnable & Fixed & Masking & Buffer\\
    \midrule
    \texttt{NELE} & $l$, $w_1$, $w_2$, $y_1$, $x_1$, $y_0$ & $w_0$, $w_3$, $x_0$ = $min(x)$ & $x < 0$ & no\\
    \texttt{NELE\_LEARNABLE} & $l$, $w_1$, $w_2$, $y_1$, $x_1$, $y_0$ & $w_0$, $w_3$, $x_0$ & $x \leq 0$ & yes\\
    \texttt{NELE\_PARAM} & - & $p_1$, $p_2$, $p_3$, $w_0$, $w_1$, $w_2$, $w_3$, $x_0$ = $min(x)$& $x\leq0$ and $x>x_0$ & yes\\
    \texttt{NELE\_PARAM\_DIR} & - & $p_0$, $p_1$, $p_2$, $p_3$, $w_0$, $w_1$, $w_2$, $w_3$ & $x\leq 0$ and $x \geq x_0$ & yes\\
  \bottomrule
\end{tabular}
\end{table}
\begin{algorithm}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=0.45\linewidth]{../../PICS/Approx_gelu.pdf}
