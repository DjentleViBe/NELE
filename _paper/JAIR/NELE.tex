...
Figure ~\ref{fig:nurbs} illustrates how varying the weight of the middle control 
  ...
  spline, there are 4 control points and their asociated weights. For the learnable version of the 
  activatin, a piecewise type construction is considered.
 ...
 Other variations are listed in Table~\ref{tab:nelevars}.
  \begin{table}
...
\FOR{each batch}
    ...
    \ENDIF
    \STATE Evaluate 
            $N_0 = (1-t)^3$, $N_1 = 3t(1-t)^2$, $N_2 = 3t^2(1-t)$, $N_3=t^3$
    \STATE Calculate $\text{Numerator\_y} = \sum N_n * w_n * y_n$, $\text{Numerator\_x} = \sum N_n * w_n * x_n$, $\text{Denominator} = \sum N_n * w_n$
    \STATE Calculate $\text{Numerator\_y}/\text{Denominator}$. $\text{Numerator\_x}/\text{Denominator}$.
    \STATE Find $y$ for $\mathcal{D}$ values via Linear Interpolation
    \IF{Masking}
      \IF{Double}
