...
\begin{itemize}
\item Universality : To enable an AF which opened up a search 
...
NURBS-based construction Appendix~\ref{app:nurbsparams}. Attenuation with non-zero gradients 
...
A cubic spline is employed as it provides sufficient expressivity while maintaining low computational complexity and numerical stability.
...
overfitting and their associated weights.  
The implementation is summarized in Algorithm ~\ref{alg:nele_training}.
Continuity forces $p_3 = [0.0, 0.0]$ and linearity of the positive side 
sets the slope between $p_2$ and $p_3$ = 1 while maintaining tangency as depicted in Figure ~\ref{fig:nurbsregen}.
The remaining control points and their associated weights were treated as learnable parameters. 
For the fixed variant, limiting $p_0$ to [-4.0, 0.0] and setting all weights 
to 1 yielded the best results. 
\begin{figure}[H]
  \begin{center}
